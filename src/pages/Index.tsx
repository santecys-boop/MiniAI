...
export default function Index() {
  ...
  // Google & GitHub OAuth Return Parser
  useEffect(() => {
    // 1. GitHub OAuth Code Callback
    if (window.location.search && window.location.search.includes("code=")) {
      try {
        ...
        if (code) {
          ...
          const githubUser = {
            id: "gh_" + Math.random().toString(36).slice(2, 10),
            name: "GitHub Developer",
            email: "developer@github.com",
            avatar: "https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png",
            provider: "github"
          };
          localStorage.setItem("mini_ai_github_user", JSON.stringify(githubUser));
          toast.dismiss();
          toast.success("GitHub ile başarıyla giriş yapıldı! Hoş geldiniz.");
          window.history.replaceState(null, "", window.location.pathname);
          setTimeout(() => window.location.reload(), 300);
          return;
        }
      } catch (ghErr) {
        console.error("GitHub callback parse error:", ghErr);
      }
    }

    // 2. Google OAuth Return Token Parser
  ...
  async function send(opts?: { fix?: boolean; forceImage?: boolean; effort?: string; customPrompt?: string }) {
    ...
    try {
      ...
      if ((!finalChat || finalChat.trim().length === 0) && finalToolCheck.questions && finalToolCheck.questions.length > 0) {
        finalChat = "✨ Uygulamanızı en iyi şekilde tasarlamak için aşağıdaki kısa anketi doldurabilir veya doğrudan başlatabilirsiniz:";
      }
