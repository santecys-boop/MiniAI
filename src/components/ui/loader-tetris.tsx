...
const PALETTE: Record<number, string> = {
    ...
    6: "#60a5fa", // J - blue
    7: "#fb923c", // L - orange
    8: "#ffffff", // line clear flash
    9: "rgba(160, 160, 160, 0.45)", // dead
};
...
export function TetrisLoader({
    ...
    const paint = useCallback(
        (dots: HTMLDivElement[], index: number) => {
            ...
            dots.forEach((dot, i) => {
                const val = board[i] ?? 0;
                if (val && PALETTE[val]) {
                    dot.style.backgroundColor = PALETTE[val];
                    dot.style.opacity = "1";
                } else {
    ...
    return (
        <div
            ...
            aria-label={label}
            aria-busy={playing}
            className={cn("grid w-fit p-1.5 rounded-lg bg-black/40 border border-white/10 shadow-inner", className)}
            style={
                {
                    gridTemplateColumns: `repeat(${width}, ${size(cellSize)})`,
                    gap: size(gap),
                    ...style,
                } as CSSProperties
            }
        ...
        >
            {Array.from({ length: width * height }).map((_, i) => (
                <div
                    ...
                    className={cn("bg-white/10 transition-colors duration-75", dotClassName)}
                />
            ))}
        </div>
    );
